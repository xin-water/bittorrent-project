...
pub fn pieces_spilt(downloader_mutex: Arc<Mutex<Download>>) {
    ...
    let (file_length, piece_length, num_pieces, mut piece_list) = {
        ...
        (torrent.info.size.unwrap(), torrent.info.piece_length, torrent.info.num_pieces.unwrap(), torrent.info.piece_list.clone().unwrap())
    };
    ...
    let time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() - start;
    println!("\n分割用时:{}", time);
...
impl Download {
    ...
    pub fn store(&mut self, socket_addr: SocketAddr, download_size: f64, piece_index: u32, block_index: u32, data: Vec<u8>) -> Result<(), Error> {
        self.active_connext.insert(socket_addr.clone(), download_size);
        {
        ...
        if self.pieces[piece_index as usize].is_complete {
            ...
            println!("已下载{}块 - {} MB", self.download_paces, (self.download_size as f64) / 1024.0 / 1024.0);
        }
