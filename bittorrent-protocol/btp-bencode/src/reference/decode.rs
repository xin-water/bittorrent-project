...
pub fn decode<'a>(
    ...
    opts: BDecodeOpt,
    depth: usize,
) -> Result<(BencodeRef<'a>, usize), BencodeParseError> {
    ...
    match curr_byte {
        ...
        _ => Err(BencodeParseError::from_kind(
            BencodeParseErrorKind::InvalidByte { pos: pos },
        )),
    }
}
...
fn decode_int<'a>(bytes: &'a [u8], pos: usize, delim: u8) -> BencodeParseResult<(i64, usize)> {
    ...
    let relative_end_pos = match begin_decode.iter().position(|n| *n == delim) {
        ...
        None => {
            return Err(BencodeParseError::from_kind(
                BencodeParseErrorKind::InvalidIntNoDelimiter { pos: pos },
            ))
        }
    };
...
fn decode_bytes<'a>(bytes: &'a [u8], pos: usize) -> BencodeParseResult<(&'a [u8], usize)> {
    ...
    if num_bytes < 0 {
        return Err(BencodeParseError::from_kind(
            BencodeParseErrorKind::InvalidLengthNegative { pos: pos },
        ));
    }
    ...
    if num_bytes > bytes[start_pos..].len() {
        return Err(BencodeParseError::from_kind(
            BencodeParseErrorKind::InvalidLengthOverflow { pos: pos },
        ));
    }
...
fn decode_dict<'a>(
    ...
    while curr_byte != crate::BEN_END {
        ...
        let (value, next_pos) = decode(bytes, curr_pos, opts, depth + 1).unwrap();
...
mod tests {
    ...
    fn positive_decode_int() {
        let int_value = super::decode_int(INT, 1, crate::BEN_END)
            .unwrap()
            .0;
    ...
    fn positive_decode_int_zero() {
        let int_value = super::decode_int(INT_ZERO, 1, crate::BEN_END)
            .unwrap()
            .0;
    ...
    fn negative_decode_int_nan() {
        super::decode_int(INT_NAN, 1, crate::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_leading_zero() {
        super::decode_int(INT_LEADING_ZERO, 1, crate::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_double_zero() {
        super::decode_int(INT_DOUBLE_ZERO, 1, crate::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_negative_zero() {
        super::decode_int(INT_NEGATIVE_ZERO, 1, crate::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_double_negative() {
        super::decode_int(INT_DOUBLE_NEGATIVE, 1, crate::BEN_END)
            .unwrap()
            .0;
    }
