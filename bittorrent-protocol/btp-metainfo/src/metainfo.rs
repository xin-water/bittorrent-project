...
use btp_bencode::{BDecodeOpt, BDictAccess, BRefAccess, BencodeRef};
use btp_util::bt::InfoHash;
use btp_util::sha::{self, ShaHash};
...
mod tests {
    ...
    fn validate_parse_from_params(
        ...
        let info_hash = {
            ...
            tracker
                .map(|t| root_dict_access.insert(parse::ANNOUNCE_URL_KEY.into(), ben_bytes!(t)));
            create_date
                .as_ref()
            ...
            create_by
                .map(|c| root_dict_access.insert(parse::CREATED_BY_KEY.into(), ben_bytes!(c)));
            ...
            let mut info_dict = BencodeMut::new_dict();
            {
                ...
                piece_length.as_ref().map(|&p| {
                    info_dict_access.insert(parse::PIECE_LENGTH_KEY.into(), ben_int!(p))
                });
