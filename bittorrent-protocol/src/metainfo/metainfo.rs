...
use super::builder::{InfoBuilder, MetainfoBuilder, PieceLength};
...
mod tests {
    ...
    fn validate_parse_from_params(
        ...
        let info_hash = {
            ...
            tracker
                .map(|t| root_dict_access.insert(parse::ANNOUNCE_URL_KEY.into(), bt_ben_bytes!(t)));
            create_date
                .as_ref()
                .map(|&c| root_dict_access.insert(parse::CREATION_DATE_KEY.into(), bt_ben_int!(c)));
            comment.map(|c| root_dict_access.insert(parse::COMMENT_KEY.into(), bt_ben_bytes!(c)));
            create_by
                .map(|c| root_dict_access.insert(parse::CREATED_BY_KEY.into(), bt_ben_bytes!(c)));
            encoding.map(|e| root_dict_access.insert(parse::ENCODING_KEY.into(), bt_ben_bytes!(e)));

            let mut info_dict = BencodeMut::new_dict();
            {
                let info_dict_access = info_dict.dict_mut().unwrap();

                piece_length.as_ref().map(|&p| {
                    info_dict_access.insert(parse::PIECE_LENGTH_KEY.into(), bt_ben_int!(p))
                });
                pieces.map(|p| info_dict_access.insert(parse::PIECES_KEY.into(), bt_ben_bytes!(p)));
                private
                    .as_ref()
                    .map(|&p| info_dict_access.insert(parse::PRIVATE_KEY.into(), bt_ben_int!(p)));

                directory
                    .and_then(|d| {
                        // We intended to build a multi file torrent since we provided a directory
                        info_dict_access.insert(parse::NAME_KEY.into(), bt_ben_bytes!(d));
...

                            {
                                ...
                                for &(ref opt_len, ref opt_md5, ref opt_paths) in files.iter() {
                                    let opt_bencode_paths = opt_paths.as_ref().map(|paths| {
...

                                        {
                                            ...
                                            for path in paths.iter() {
                                                bencode_paths_access.push(bt_ben_bytes!(&path[..]));
                                            }
                                        }
                                    ...
                                    let mut file_dict = BencodeMut::new_dict();
                                    {
                                        ...
                                        opt_len.map(|l| {
                                            file_dict_access
                                                .insert(parse::LENGTH_KEY.into(), bt_ben_int!(l))
                                        });
                                        opt_md5.map(|m| {
                                            file_dict_access
                                                .insert(parse::MD5SUM_KEY.into(), bt_ben_bytes!(m))
                                        });
                                    }
                    ...
                    .or_else(|| {
                        ...
                        files.as_ref().map(|files| {
                            ...
                            opt_path.as_ref().map(|p| {
                                info_dict_access
                                    .insert(parse::NAME_KEY.into(), bt_ben_bytes!(&p[0][..]))
                            });
                            opt_len.map(|l| {
                                info_dict_access.insert(parse::LENGTH_KEY.into(), bt_ben_int!(l))
                            });
                            opt_md5.map(|m| {
                                info_dict_access.insert(parse::MD5SUM_KEY.into(), bt_ben_bytes!(m))
                            });
                        });
