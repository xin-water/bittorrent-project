...
use crate::metainfo::builder::buffer::{PieceBuffer, PieceBuffers};
use crate::metainfo::error::ParseResult;
use crate::metainfo::{Accessor, PieceAccess};
    ...
    C: FnMut(f64) + Send + 'static,
{
    ...
    let work_queue = Arc::new(Mutex::new(SegQueue::new()));

    // Create buffer allocator to reuse pre allocated buffers
    let piece_buffers = Arc::new(Mutex::new(PieceBuffers::new(piece_length, num_workers)));
...
fn start_hash_master<A>(
    ...
    work: Arc<Mutex<SegQueue<WorkerMessage>>>,
    buffers: Arc<Mutex<PieceBuffers>>,
    ...
    A: Accessor,
{
    ...
    accessor.access_pieces(|piece_access| {
        match piece_access {
            PieceAccess::Compute(piece_region) => {
                ...
                let mut curr_piece_buffer = PieceBuffer::new(1);

                if let Some(piece_buffer) = opt_piece_buffer.take() {
                    curr_piece_buffer = piece_buffer;
                } else {
                    loop {
                        let temp={buffers.lock().unwrap().checkout()};
                        if temp.is_some() {
                            curr_piece_buffer = temp.unwrap();
                            break
                        }
                    }
                };
                ...
                while !end_of_region {
                    ...
                    if curr_piece_buffer.is_whole() {
                        work.lock().unwrap().push(WorkerMessage::HashPiece(piece_index, curr_piece_buffer));
    ...
    if let Some(piece_buffer) = opt_piece_buffer {
        if !piece_buffer.is_empty() {
            work.lock().unwrap().push(WorkerMessage::HashPiece(piece_index, piece_buffer));
    ...
    for _ in 0..num_workers {
        work.lock().unwrap().push(WorkerMessage::Finish);
    }
    ...
    while workers_finished < num_workers {
        match recv.recv() {
            ...
            Err(_) => panic!(
                "bittorrent-protocol_metainfo: Master failed to verify all workers shutdown..."
            ),
        }
    }
...
fn start_hash_worker(
    ...
    while work_to_do {
        let work_item = work.lock().unwrap().pop();
        ...
        match work_item.unwrap() {
            ...
            WorkerMessage::HashPiece(index, buffer) => {
                ...
                buffers.lock().unwrap().checkin(buffer);
            }
        }
    }
...
mod tests {
    ...
    use crate::metainfo::builder::worker;
    use crate::metainfo::{Accessor, PieceAccess};
    use crate::util::sha::ShaHash;
