                    ...
                    for group in groups.iter() {
...

                        {
                            ...
                            for tracker_url in group.iter() {
                                tracker_list_access.push(bt_ben_bytes!(&tracker_url[..]));
                            }
                        }
    ...
    pub fn set_main_tracker(mut self, opt_tracker_url: Option<&'a str>) -> MetainfoBuilder<'a> {
        {
            ...
            if let Some(tracker_url) = opt_tracker_url {
                dict_access.insert(parse::ANNOUNCE_URL_KEY.into(), bt_ben_bytes!(tracker_url));
            } else {
    ...
    pub fn set_creation_date(mut self, opt_secs_epoch: Option<i64>) -> MetainfoBuilder<'a> {
        {
            ...
            if let Some(secs_epoch) = opt_secs_epoch {
                dict_access.insert(parse::CREATION_DATE_KEY.into(), bt_ben_int!(secs_epoch));
            } else {
    ...
    pub fn set_comment(mut self, opt_comment: Option<&'a str>) -> MetainfoBuilder<'a> {
        {
            ...
            if let Some(comment) = opt_comment {
                dict_access.insert(parse::COMMENT_KEY.into(), bt_ben_bytes!(comment));
            } else {
    ...
    pub fn set_created_by(mut self, opt_created_by: Option<&'a str>) -> MetainfoBuilder<'a> {
        {
            ...
            if let Some(created_by) = opt_created_by {
                dict_access.insert(parse::CREATED_BY_KEY.into(), bt_ben_bytes!(created_by));
            } else {
...

        {
            ...
            opt_numeric_is_private
                .and_then(|numeric_is_private| {
                    dict_access.insert(parse::PRIVATE_KEY.into(), bt_ben_int!(numeric_is_private))
                })
...
{
    if threads == 0 {
        panic!("bittorrent-protocol_metainfo: Cannot Build Metainfo File With threads == 0");
    }
    ...
    // Update the info bencode with values
    {
        ...
        info_access.insert(
            ...
            bt_ben_int!(piece_length as i64),
        );
        info_access.insert(parse::PIECES_KEY.into(), bt_ben_bytes!(&pieces[..]));
        ...
        match (&access_directory, files_info.len() > 1) {
                    ...
                    for &(len, ref path) in files_info.iter() {
...

                        {
                            ...
                            for path_element in path.iter() {
                                bencode_path_access.push(bt_ben_bytes!(&path_element[..]));
                            }
                        }

                        bencode_files_access.push(bt_ben_map! {
                            parse::LENGTH_KEY => bt_ben_int!(len as i64),
                            parse::PATH_KEY   => bencode_path
                        });
                    }
                }

                info_access.insert(parse::NAME_KEY.into(), bt_ben_bytes!(directory.as_ref()));
            ...
            (&None, true) => {
                ...
                info_access.insert(parse::NAME_KEY.into(), bt_ben_bytes!(""));
                info_access.insert(parse::FILES_KEY.into(), bencode_files);
            }
            (&None, false) => {
                // Single File
                ...
                info_access.insert(
                    parse::LENGTH_KEY.into(),
                    bt_ben_int!(files_info[0].0 as i64),
                );
                info_access.insert(parse::NAME_KEY.into(), bt_ben_bytes!(&single_file_name[..]));
            }
        }
    }
