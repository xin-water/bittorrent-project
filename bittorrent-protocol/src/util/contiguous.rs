    ...
    T: Clone,
{
    ...
    fn write(&mut self, data: &[T]) {
        ...
        if data.len() > available_space {
            panic!("bittorrent-protocol_util: ContiguousBuffer::write Detected Write That Overflows Vec");
        } else {
    ...
    I: Clone,
{
    ...
    fn write(&mut self, data: &[I]) {
        ...
        if bytes_written != data.len() {
            panic!("bittorrent-protocol_util: ContiguousBuffer::write Detected Write That Overflows ContiguousBuffers");
        }
    }
