...
use crate::utracker::announce::AnnounceResponse;
use crate::utracker::contact::CompactPeers;
use crate::utracker::error::ErrorResponse;
        ...
        W: Write,
    {
        match self.response_type() {
            &ResponseType::Connect(id) => {
                writer.write_u32::<BigEndian>(crate::utracker::CONNECT_ACTION_ID)?;
            ...
            &ResponseType::Announce(ref req) => {
                let action_id = match req.peers() {
                    &CompactPeers::V4(_) => crate::utracker::ANNOUNCE_IPV4_ACTION_ID,
                    &CompactPeers::V6(_) => crate::utracker::ANNOUNCE_IPV6_ACTION_ID,
                };
            ...
            &ResponseType::Scrape(ref req) => {
                writer.write_u32::<BigEndian>(crate::utracker::SCRAPE_ACTION_ID)?;
                writer.write_u32::<BigEndian>(self.transaction_id())?;
...
fn parse_response<'a>(bytes: &'a [u8]) -> IResult<&'a [u8], TrackerResponse<'a>> {
    switch!(bytes, tuple!(be_u32, be_u32),
        (crate::utracker::CONNECT_ACTION_ID, tid)  => map!(be_u64, |cid| TrackerResponse::new(tid, ResponseType::Connect(cid)) ) |
        (crate::utracker::ANNOUNCE_IPV4_ACTION_ID, tid) => map!(call!(AnnounceResponse::from_bytes_v4), |ann_res| {
            TrackerResponse::new(tid, ResponseType::Announce(ann_res))
        }) |
        (crate::utracker::SCRAPE_ACTION_ID, tid)   => map!(call!(ScrapeResponse::from_bytes), |scr_res| {
        ...
        (crate::utracker::ANNOUNCE_IPV6_ACTION_ID, tid) => map!(call!(AnnounceResponse::from_bytes_v6), |ann_req| {
