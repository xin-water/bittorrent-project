...
use crate::utracker::announce::AnnounceRequest;
use crate::utracker::scrape::ScrapeRequest;
        ...
        match self.request_type() {
            ...
            &RequestType::Announce(ref req) => {
                let action_id = if req.source_ip().is_ipv4() {
                    crate::utracker::ANNOUNCE_IPV4_ACTION_ID
                } else {
                    crate::utracker::ANNOUNCE_IPV6_ACTION_ID
                };
...
fn parse_request<'a>(bytes: &'a [u8]) -> IResult<&'a [u8], TrackerRequest<'a>> {
    switch!(bytes, tuple!(be_u64, be_u32, be_u32),
        (CONNECT_ID_PROTOCOL_ID, crate::utracker::CONNECT_ACTION_ID, tid) => value!(
            TrackerRequest::new(CONNECT_ID_PROTOCOL_ID, tid, RequestType::Connect)
        ) |
        (cid, crate::utracker::ANNOUNCE_IPV4_ACTION_ID, tid) => map!(call!(AnnounceRequest::from_bytes_v4), |ann_req| {
            TrackerRequest::new(cid, tid, RequestType::Announce(ann_req))
        }) |
        (cid, crate::utracker::SCRAPE_ACTION_ID, tid) => map!(call!(ScrapeRequest::from_bytes), |scr_req| {
            TrackerRequest::new(cid, tid, RequestType::Scrape(scr_req))
        }) |
        (cid, crate::utracker::ANNOUNCE_IPV6_ACTION_ID, tid) => map!(call!(AnnounceRequest::from_bytes_v6), |ann_req| {
            TrackerRequest::new(cid, tid, RequestType::Announce(ann_req))
        })
    )
}
