...
impl UtpSocket {
    ...
    pub fn connect<A: ToSocketAddrs>(other: A) -> Result<UtpSocket> {
        ...
        for _ in 0..MAX_SYN_RETRIES {
            ...
            // Send packet
            debug!("start connecting to {}", socket.connected_to);
    ...
    fn flush_incoming_buffer(&mut self, buf: &mut [u8]) -> usize {
        ...
        if !self.pending_data.is_empty() {
            ...
            if flushed == self.pending_data.len() {
                ...
                if let Some(package) = self.advance_incoming_buffer(){
                    self.pending_data=package.payload().to_vec();
                }
            } else {
    ...
    pub fn send_to(&mut self, buf: &[u8]) -> Result<usize> {
        ...
        for chunk in buf.chunks(MSS as usize - HEADER_SIZE) {
            ...
            packet.set_wnd_size(WINDOW_SIZE - self.send_window.len() as u32);
            packet.set_seq_nr(self.seq_nr);
            packet.set_ack_nr(self.ack_nr);
            packet.set_connection_id(self.sender_connection_id);
    ...
    fn handle_state_packet(&mut self, packet: &Packet) {
        ...
        if let Some(index) = self.send_window.iter().position(|p| {
            debug!("handle_state_packet : packet.ack_nr() = {:?} , p.seq_nr() = {:?}",packet.ack_nr(),p.seq_nr());
