...
use crate::handshake::HandshakeFilter;
...
impl Filters {
        ...
        B: FnOnce(&[Box<dyn HandshakeFilter + Send + Sync>]) -> R,
    {
        let ref_filters = self
            ...
            .as_ref()
            .read()
            .expect("bittorrent-protocol_handshake: Poisoned Read Lock In Filters");
        ...
        B: FnOnce(&mut Vec<Box<dyn HandshakeFilter + Send + Sync>>) -> R,
    {
        let mut mut_filters = self
            ...
            .as_ref()
            .write()
            .expect("bittorrent-protocol_handshake: Poisoned Write Lock In Filters");
...
pub mod test_filters {
    ...
    use crate::handshake::{FilterDecision, HandshakeFilter, Protocol};
