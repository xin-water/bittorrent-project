...
use crate::select::ut_metadata::{IUtMetadataMessage, OUtMetadataMessage};
...
impl UtMetadataModule {
    ...
    fn add_torrent(
        &mut self,
        metainfo: Metainfo,
    ) -> Result<Option<IUtMetadataMessage>, UtMetadataError> {
        let info_hash = metainfo.info().info_hash();

        match self.completed_map.entry(info_hash) {
            Entry::Occupied(_) => Err(UtMetadataError::from_kind(
                UtMetadataErrorKind::InvalidMetainfoExists { hash: info_hash },
            )),
            Entry::Vacant(vac) => {
    ...
    fn remove_torrent(
        ...
        if self
            ...
            .is_none()
        {
            Err(UtMetadataError::from_kind(
                UtMetadataErrorKind::InvalidMetainfoNotExists {
    ...
    fn remove_peer(&mut self, info: PeerInfo) -> Result<Option<IUtMetadataMessage>, UtMetadataError> {
    ...
    fn recv_request(
        ...
        if self.peer_requests.len() == MAX_PEER_REQUESTS {
            Ok(Some(IUtMetadataMessage::ReceivedUtMetadataMessage(
                info,
    ...
    fn retrieve_completed_download(&mut self) -> Option<Result<OUtMetadataMessage, UtMetadataError>> {
        ...
        opt_completed_hash.and_then(|completed_hash| {
            ...
            match Info::from_bytes(&completed.bytes[..]) {
                Ok(info) => Some(Ok(OUtMetadataMessage::DownloadedMetainfo(info.into()))),
                Err(_) => self.retrieve_completed_download(),
            }
        })
    }

    fn retrieve_piece_request(&mut self) -> Option<Result<OUtMetadataMessage, UtMetadataError>> {
        ...
        for (hash, opt_pending) in self.pending_map.iter_mut() {
            ...
            if has_ready_requests && has_active_peers {
                ...
                return Some(Ok(OUtMetadataMessage::SendUtMetadataMessage(
                    *selected_peer,
    ...
    fn retrieve_piece_response(&mut self) -> Option<Result<OUtMetadataMessage, UtMetadataError>> {
...
impl UtMetadataModule {
    ...
    pub fn send(&mut self, item: IUtMetadataMessage) -> Result<Option<IUtMetadataMessage>, UtMetadataError> {
        let start_send = match item {
            IUtMetadataMessage::Control(ControlMessage::AddTorrent(metainfo)) => {
                self.add_torrent(metainfo)
            }
            IUtMetadataMessage::Control(ControlMessage::RemoveTorrent(metainfo)) => {
                self.remove_torrent(metainfo)
            }
            // Dont add the peer yet, use listener to get notified when they send extension messages
            IUtMetadataMessage::Control(ControlMessage::PeerConnected(_)) => Ok(None),
            IUtMetadataMessage::Control(ControlMessage::PeerDisconnected(info)) => {
                self.remove_peer(info)
            }
            IUtMetadataMessage::Control(ControlMessage::Tick(duration)) => self.apply_tick(duration),
            IUtMetadataMessage::DownloadMetainfo(hash) => self.download_metainfo(hash),
            IUtMetadataMessage::ReceivedUtMetadataMessage(info, UtMetadataMessage::Request(msg)) => {
                self.recv_request(info, msg)
            }
            IUtMetadataMessage::ReceivedUtMetadataMessage(info, UtMetadataMessage::Data(msg)) => {
                self.recv_data(info, msg)
            }
            IUtMetadataMessage::ReceivedUtMetadataMessage(info, UtMetadataMessage::Reject(msg)) => {
    ...
    pub fn poll(&mut self) -> Option<Result<OUtMetadataMessage, UtMetadataError>> {
        ...
        let opt_result = self
            ...
            .or_else(||
                self.retrieve_piece_request()
            )
            .or_else(||
                self.retrieve_piece_response()
            );
