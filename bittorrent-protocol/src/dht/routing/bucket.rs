...
mod tests {
    ...
    fn positive_all_questionable_nodes() {
        ...
        let dummy_addr = util_test::dummy_socket_addr_v4();
        let dummy_ids = util_test::dummy_block_node_ids(super::MAX_BUCKET_SIZE as u8);
    ...
    fn positive_resist_good_node_churn() {
        ...
        let dummy_addr = util_test::dummy_socket_addr_v4();
        let dummy_ids = util_test::dummy_block_node_ids((super::MAX_BUCKET_SIZE as u8) + 1);
        for index in 0..super::MAX_BUCKET_SIZE {
            let node = Node::as_good(dummy_ids[index], dummy_addr);
            bucket.add_node(node);
        }
