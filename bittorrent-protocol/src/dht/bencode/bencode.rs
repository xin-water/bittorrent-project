...
use crate::dht::bencode::{
    decode, encode, BencodeParseError, BencodeParseErrorKind, BencodeParseResult, Dictionary,
};
...
pub enum BencodeKind<'b, 'a: 'b> {
    ...
    List(&'b [Bencode<'a>]),
    /// Bencode Dictionary.
    Dict(&'b dyn Dictionary<'a, Bencode<'a>>),
}
...
impl<'a> Bencode<'a> {
    pub fn decode(bytes: &'a [u8]) -> BencodeParseResult<Bencode<'a>> {
        ...
        let (bencode, end_pos) = decode::decode(bytes, 0)?;

        if end_pos != bytes.len() {
            return Err(BencodeParseError::from_kind(
                BencodeParseErrorKind::BytesEmpty { pos: Some(end_pos) },
            ));
        }
    ...
    pub fn dict(&self) -> Option<&dyn Dictionary<'a, Bencode<'a>>> {
        match self {
