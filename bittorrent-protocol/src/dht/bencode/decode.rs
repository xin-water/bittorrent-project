...
use crate::dht::bencode::{Bencode, BencodeParseError, BencodeParseErrorKind, BencodeParseResult};
...
pub fn decode<'a>(bytes: &'a [u8], mut pos: usize) -> BencodeParseResult<(Bencode<'a>, usize)> {
    ...
    let (shallow_type, new_pos) = decode_shallow(bytes, pos)?;
    pos = new_pos;
    ...
    while let Some(curr_ibencode) = bencode_stack.pop() {
        match curr_ibencode {
            ib_int @ IBencodeType::Bencode(Bencode::Int(_)) => {
                let opt_last_type = collapse_bencode(&mut bencode_stack, ib_int, pos)?;
            ...
            ib_bytes @ IBencodeType::Bencode(Bencode::Bytes(_)) => {
                let opt_last_type = collapse_bencode(&mut bencode_stack, ib_bytes, pos)?;
            ...
            ib_list @ IBencodeType::Bencode(Bencode::List(_)) => {
                ...
                if curr_byte == crate::dht::bencode::BEN_END {
                    // Recursive type has reached it's end, collapse it now
                    pos += 1;
                    let opt_last_type = collapse_bencode(&mut bencode_stack, ib_list, pos)?;
            ...
            ib_dict @ IBencodeType::Bencode(Bencode::Dict(_)) => {
                ...
                if curr_byte == crate::dht::bencode::BEN_END {
                    // Recursive type has reached it's end, collapse it now
                    pos += 1;
                    let opt_last_type = collapse_bencode(&mut bencode_stack, ib_dict, pos)?;
                    ...
                    let (key, new_pos) = decode_key(bytes, pos)?;
                    pos = new_pos;
            ...
            ib_map_int @ IBencodeType::BencodeMapping(_, Bencode::Int(_)) => {
                let opt_last_type = collapse_bencode(&mut bencode_stack, ib_map_int, pos)?;
            ...
            ib_map_bytes @ IBencodeType::BencodeMapping(_, Bencode::Bytes(_)) => {
                let opt_last_type = collapse_bencode(&mut bencode_stack, ib_map_bytes, pos)?;
            ...
            ib_map_list @ IBencodeType::BencodeMapping(_, Bencode::List(_)) => {
                ...
                if curr_byte == crate::dht::bencode::BEN_END {
                    // Recursive type has reached it's end, collapse it now
                    pos += 1;
                    let opt_last_type = collapse_bencode(&mut bencode_stack, ib_map_list, pos)?;
            ...
            ib_map_dict @ IBencodeType::BencodeMapping(_, Bencode::Dict(_)) => {
                ...
                if curr_byte == crate::dht::bencode::BEN_END {
                    // Recursive type has reached it's end, collapse it now
                    pos += 1;
                    let opt_last_type = collapse_bencode(&mut bencode_stack, ib_map_dict, pos)?;
    ...
    panic!("bittorrent-protocol_bencode: Reached End Of Decode Without Returning")
}
...
fn collapse_bencode<'a>(
    stack: &mut Vec<IBencodeType<'a>>,
    ibencode: IBencodeType<'a>,
    curr_pos: usize,
) -> BencodeParseResult<Option<Bencode<'a>>> {
    if let Some(top_ibencode) = stack.pop() {
        match (top_ibencode, ibencode) {
            (IBencodeType::Bencode(Bencode::Int(_)), _) => panic!("bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Into A Bencode::Int"),
            (IBencodeType::Bencode(Bencode::Bytes(_)), _) => {
                panic!("bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Into A Bencode::Bytes")
            }
            ...
            (IBencodeType::Bencode(Bencode::List(_)), IBencodeType::BencodeMapping(..)) => panic!(
                "bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Mapping Into A \
                        Bencode::List"
            ),
            ...
            (IBencodeType::Bencode(Bencode::Dict(_)), IBencodeType::Bencode(_)) => {
                panic!("bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Into A Bencode::Dict")
            }
            (IBencodeType::BencodeMapping(_, Bencode::Int(_)), _) => panic!(
                "bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Into A Bencode::Int \
                        Mapping"
            ),
            (IBencodeType::BencodeMapping(_, Bencode::Bytes(_)), _) => panic!(
                "bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Into A Bencode::Bytes \
                        Mapping"
            ),
            ...
            (IBencodeType::BencodeMapping(_, Bencode::List(_)), IBencodeType::BencodeMapping(..)) => panic!(
                "bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Mapping Into A \
            ...
            (IBencodeType::BencodeMapping(_, Bencode::Dict(_)), IBencodeType::Bencode(_)) => panic!(
                "bittorrent-protocol_bencode: Attempted To Collapse A Bencode Type Into A Bencode::Dict \
                        Mapping"
            ),
        }
    } else {
        ...
        match ibencode {
            ...
            IBencodeType::BencodeMapping(..) => {
                panic!("bittorrent-protocol_bencode: Got BencodeMapping, Expected Bencode")
            }
        }
    }
}
...
fn decode_shallow<'a>(bytes: &'a [u8], pos: usize) -> BencodeParseResult<(Bencode<'a>, usize)> {
    ...
    match curr_byte {
        crate::dht::bencode::INT_START => {
            let (bencode, pos) = decode_int(bytes, pos + 1, crate::dht::bencode::BEN_END)?;
            Ok((Bencode::Int(bencode), pos))
        }
        crate::dht::bencode::LIST_START => Ok((Bencode::List(Vec::new()), pos + 1)),
        crate::dht::bencode::DICT_START => Ok((Bencode::Dict(BTreeMap::new()), pos + 1)),
        crate::dht::bencode::BYTE_LEN_LOW..=crate::dht::bencode::BYTE_LEN_HIGH => {
            let (bencode, pos) = decode_bytes(bytes, pos)?;
            // Include the length digit, don't increment position
            Ok((Bencode::Bytes(bencode), pos))
        }
        _ => Err(BencodeParseError::from_kind(
            BencodeParseErrorKind::InvalidByte { pos: Some(pos) },
        )),
    }
}
...
fn decode_int(bytes: &[u8], pos: usize, delim: u8) -> BencodeParseResult<(i64, usize)> {
    ...
    let relative_end_pos = match begin_decode.iter().position(|n| *n == delim) {
        ...
        None => {
            return Err(BencodeParseError::from_kind(
                BencodeParseErrorKind::InvalidIntNoDelimiter { pos: Some(pos) },
            ))
        }
    };
    ...
    if int_byte_slice.len() > 1 {
        ...
        if int_byte_slice[0] == b'-' && int_byte_slice[1] == b'0' {
            return Err(BencodeParseError::from_kind(
                BencodeParseErrorKind::InvalidIntNegativeZero { pos: Some(pos) },
            ));
        }
        ...
        if int_byte_slice[0] == b'0' {
            return Err(BencodeParseError::from_kind(
                BencodeParseErrorKind::InvalidIntZeroPadding { pos: Some(pos) },
            ));
        }
    }
    ...
    let int_str = match str::from_utf8(int_byte_slice) {
        ...
        Err(_) => {
            return Err(BencodeParseError::from_kind(
                BencodeParseErrorKind::InvalidIntParseError { pos: Some(pos) },
            ))
        }
    };
...
fn decode_bytes<'a>(bytes: &'a [u8], pos: usize) -> BencodeParseResult<(&'a [u8], usize)> {
    let (num_bytes, start_pos) = decode_int(bytes, pos, crate::dht::bencode::BYTE_LEN_END)?;

    if num_bytes < 0 {
        return Err(BencodeParseError::from_kind(
            BencodeParseErrorKind::InvalidLengthNegative { pos: Some(pos) },
        ));
    }
    ...
    if num_bytes > bytes[start_pos..].len() {
        return Err(BencodeParseError::from_kind(
            BencodeParseErrorKind::InvalidLengthOverflow { pos: Some(pos) },
        ));
    }
...
fn decode_key<'a>(bytes: &'a [u8], pos: usize) -> BencodeParseResult<(&'a [u8], usize)> {
    let (key_bytes, next_pos) = decode_bytes(bytes, pos)?;
...
fn peek_byte(bytes: &[u8], pos: usize) -> BencodeParseResult<u8> {
    bytes
        .get(pos)
        .map(|n| *n)
        .ok_or(BencodeParseError::from_kind(
            BencodeParseErrorKind::BytesEmpty { pos: Some(pos) },
        ))
}
...
mod tests {
    ...
    fn positive_decode_general() {
        ...
        assert_eq!(
            ben_dict.lookup("".as_bytes()).unwrap().str().unwrap(),
            "zero_len_key"
        );
        assert_eq!(
            ben_dict
                .lookup("location".as_bytes())
                .unwrap()
                .str()
                .unwrap(),
            "udp://test.com:80"
        ...
        assert_eq!(
            ben_dict.lookup("number".as_bytes()).unwrap().int().unwrap(),
            500500i64
        );

        let nested_dict = ben_dict
            .lookup("nested dict".as_bytes())
            .unwrap()
            .dict()
            .unwrap();
        let nested_list = nested_dict
            .lookup("list".as_bytes())
            .unwrap()
            .list()
            .unwrap();
    ...
    fn positive_decode_dict() {
        ...
        assert_eq!(
            dict.lookup("test_key".as_bytes()).unwrap().str().unwrap(),
            "test_value"
        );
    ...
    fn positive_decode_int() {
        let int_value = super::decode_int(INT, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
    ...
    fn positive_decode_int_negative() {
        let int_value = super::decode_int(INT_NEGATIVE, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
        assert_eq!(int_value, -500i64);
    }
    ...
    fn positive_decode_int_zero() {
        let int_value = super::decode_int(INT_ZERO, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
    ...
    fn negative_decode_int_nan() {
        super::decode_int(INT_NAN, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_leading_zero() {
        super::decode_int(INT_LEADING_ZERO, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_double_zero() {
        super::decode_int(INT_DOUBLE_ZERO, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_negative_zero() {
        super::decode_int(INT_NEGATIVE_ZERO, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
    }
    ...
    fn negative_decode_int_double_negative() {
        super::decode_int(INT_DOUBLE_NEGATIVE, 1, crate::dht::bencode::BEN_END)
            .unwrap()
            .0;
    }
