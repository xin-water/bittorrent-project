...
use crate::dht::routing::node::NodeStatus;
    ...
    H: Handshaker + 'static,
{
    ...
    thread::spawn(move || {
        if event_loop.run(&mut handler).is_err() {
            error!("bittorrent-protocol_dht: EventLoop shut down with an error...");
        }
        ...
        if kill_sock.send_to(&b"0"[..], kill_addr).is_err() {
            error!("bittorrent-protocol_dht: Failed to send a wake up message to the incoming channel...");
        }

        info!("bittorrent-protocol_dht: DhtHandler gracefully shut down, exiting thread...");
    });
    ...
    H: Handshaker,
{
    if event_loop
        ...
        .is_err()
    {
        error!("bittorrent-protocol_dht: Failed to sent a shutdown message to the EventLoop...");
    }
}
    ...
    H: Handshaker,
{
    ...
    match message {
        ...
        Ok(MessageType::Request(RequestType::GetPeers(g))) => {
            ...
            // TODO: Move socket address serialization code into use crate::util
        ...
        BootstrapStatus::Completed => {
            ...
            if should_rebootstrap(&work_storage.routing_table) {
                let (bootstrap, attempts) = match table_actions.get_mut(&action_id) {
                    ...
                    _ => panic!("bittorrent-protocol_dht: Bug, in DhtHandler..."),
                };
    ...
    H: Handshaker,
{
    let opt_refresh_status = match table_actions.get_mut(&trans_id.action_id()) {
        ...
        Some(&mut TableAction::Lookup(_)) => {
            error!(
                "bittorrent-protocol_dht: Resolved a TransactionID to a check table refresh but TableLookup \
                    found..."
            );
            None
        }
    ...
    H: Handshaker,
{
    ...
    let opt_lookup_info = match table_actions.get_mut(&trans_id.action_id()) {
        ...
        Some(&mut TableAction::Bootstrap(_, _)) => {
            error!(
                "bittorrent-protocol_dht: Resolved a TransactionID to a check table lookup but TableBootstrap \
                    found..."
            );
            None
        }
        Some(&mut TableAction::Refresh(_)) => {
            error!(
                "bittorrent-protocol_dht: Resolved a TransactionID to a check table lookup but TableRefresh \
                    found..."
            );
            None
        }
