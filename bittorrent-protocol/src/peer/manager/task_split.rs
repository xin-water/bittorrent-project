...
use super::{IPeerManagerMessage, OPeerManagerMessage,ManagedMessage};
use crate::peer::messages::PeerWireProtocolMessage;
use crate::peer::{PeerTokioCodec, PeerWireMessageCodec};
use super::future::{
    PersistentError, PersistentStream, RecurringTimeoutError, RecurringTimeoutStream,
};
use super::builder::PeerManagerBuilder;
...
where S: AsyncRead + AsyncWrite + Send + Sized + 'static{
    ...
    let peer = PeerTokioCodec::with_max_payload(PeerWireMessageCodec::new(), 24 * 1024).framed(peer);
                    ...
                    merged_stream
                        ...
                        .then(move |result| {
                            let result = match result {
                                ...
                                Ok((Some(MergedItem::Both(IPeerManagerMessage::RemovePeer(p_info), peer_message)), merged_stream)) => Ok((
                                    merged_stream,
                                    None,
                                    Some(peer_message),
                                ...
                                Ok((Some(_), _)) => panic!("bittorrent-protocol_peer: Peer Future Received Invalid Message From Peer Manager"),
                                Err((PeerError::ManagerHeartbeatInterval, merged_stream)) => {
                                    Ok((merged_stream, Some(PeerWireProtocolMessage::keep_alive()), None, None, true))
                                }
