    ...
    fn from(error: TimeoutError<T>) -> PersistentError {
        match error {
            TimeoutError::Timer(_, err) => panic!(
                "bittorrent-protocol_peer: Timer Error In Peer Stream, Timer Capacity Is Probably Too Small: {}",
                err
            ),
    ...
    S: Stream,
{
    ...
    fn poll(&mut self) -> Poll<Option<S::Item>, RecurringTimeoutError> {
        ...
        match self.sleep.poll() {
            ...
            Err(_) => panic!("bittorrent-protocol_peer: Timer Error In Manager Stream, Timer Capacity Is Probably Too Small..."),
        }
    }
}
