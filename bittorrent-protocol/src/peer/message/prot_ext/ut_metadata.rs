...
impl UtMetadataRequestMessage {
    pub fn new(piece: i64) -> UtMetadataRequestMessage {
        let encoded_bytes_size = (bt_ben_map! {
            bencode::MESSAGE_TYPE_KEY => bt_ben_int!(REQUEST_MESSAGE_TYPE_ID as i64),
            bencode::PIECE_INDEX_KEY  => bt_ben_int!(piece)
        })
        .encode()
        .len();
        ...
        W: Write,
    {
        let encoded_bytes = (bt_ben_map! {
            bencode::MESSAGE_TYPE_KEY => bt_ben_int!(REQUEST_MESSAGE_TYPE_ID as i64),
            bencode::PIECE_INDEX_KEY  => bt_ben_int!(self.piece)
        })
        .encode();
...
impl UtMetadataDataMessage {
    pub fn new(piece: i64, total_size: i64, data: Bytes) -> UtMetadataDataMessage {
        let encoded_bytes_len = (bt_ben_map! {
            bencode::MESSAGE_TYPE_KEY => bt_ben_int!(DATA_MESSAGE_TYPE_ID as i64),
            bencode::PIECE_INDEX_KEY  => bt_ben_int!(piece),
            bencode::TOTAL_SIZE_KEY   => bt_ben_int!(total_size)
        })
        .encode()
        .len();
        ...
        W: Write,
    {
        let encoded_bytes = (bt_ben_map! {
            bencode::MESSAGE_TYPE_KEY => bt_ben_int!(DATA_MESSAGE_TYPE_ID as i64),
            bencode::PIECE_INDEX_KEY  => bt_ben_int!(self.piece),
            bencode::TOTAL_SIZE_KEY   => bt_ben_int!(self.total_size)
        })
        .encode();
...
impl UtMetadataRejectMessage {
    pub fn new(piece: i64) -> UtMetadataRejectMessage {
        let encoded_bytes_size = (bt_ben_map! {
            bencode::MESSAGE_TYPE_KEY => bt_ben_int!(REJECT_MESSAGE_TYPE_ID as i64),
            bencode::PIECE_INDEX_KEY  => bt_ben_int!(piece)
        })
        .encode()
        .len();
