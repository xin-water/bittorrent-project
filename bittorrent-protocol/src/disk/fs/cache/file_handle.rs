    ...
    F: FileSystem,
{
    ...
    {
        let mut lock_cache = self.cache.lock().expect(
            "bittorrent-protocol_disk: Failed To Lock Cache In FileHandleCache::run_with_lock",
        );
    ...
    F: FileSystem,
{
    ...
    fn file_size(&self, file: &Self::File) -> io::Result<u64> {
        let lock_file = file
            .lock()
            .expect("bittorrent-protocol_disk: Failed To Lock File In FileHandleCache::file_size");
    ...
    fn read_file(
        ...
        let mut lock_file = file
            .lock()
            .expect("bittorrent-protocol_disk: Failed To Lock File In FileHandleCache::read_file");
    ...
    fn write_file(&self, file: &mut Self::File, offset: u64, buffer: &[u8]) -> io::Result<usize> {
        let mut lock_file = file
            .lock()
            .expect("bittorrent-protocol_disk: Failed To Lock File In FileHandleCache::write_file");
