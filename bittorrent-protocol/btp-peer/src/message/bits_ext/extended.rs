...
use btp_bencode::{BConvert, BDecodeOpt, BMutAccess, BencodeRef};
use btp_util::convert;

use crate::message::{self, bencode, bits_ext};
...
fn bencode_from_builder(
...

    {
        ...
        opt_our_ip.map(|our_ip| {
            root_map_access.insert(bencode::OUR_IP_KEY.into(), ben_bytes!(our_ip))
        });
