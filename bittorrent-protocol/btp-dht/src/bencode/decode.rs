...
use crate::bencode::{Bencode, BencodeParseError, BencodeParseErrorKind, BencodeParseResult};
    ...
    while let Some(curr_ibencode) = bencode_stack.pop() {
        match curr_ibencode {
            ...
            ib_list @ IBencodeType::Bencode(Bencode::List(_)) => {
                ...
                if curr_byte == crate::bencode::BEN_END {
...
fn decode_shallow<'a>(bytes: &'a [u8], pos: usize) -> BencodeParseResult<(Bencode<'a>, usize)> {
    ...
    match curr_byte {
        crate::bencode::INT_START => {
            let (bencode, pos) = decode_int(bytes, pos + 1, crate::bencode::BEN_END)?;
            Ok((Bencode::Int(bencode), pos))
        }
        crate::bencode::LIST_START => Ok((Bencode::List(Vec::new()), pos + 1)),
        crate::bencode::DICT_START => Ok((Bencode::Dict(BTreeMap::new()), pos + 1)),
