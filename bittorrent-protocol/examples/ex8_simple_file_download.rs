...
fn main() {
    ...
    let (select_recv, piece_requests, cur_pieces) = runtime.block_on(
        future::loop_fn(
    ...
    runtime.block_on(handshaker_send.send(InitiateMessage::new(Protocol::BitTorrent, info_hash, peer_addr)).map_err(|_| ())).unwrap();
    ...
    let result: Result<(), ()> = runtime.block_on(future::loop_fn(
        (
            select_recv,
